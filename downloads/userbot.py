...
def load_config():
    default_config = {
        ...
        "api_hash": "",
        "session_string": "",
        "bot_token": "8488696730:AAHwPPhJnan8pJyHTE4xwBhWYsv_p-zjDAo",
        "time_enabled": False,
...
BOT_TOKEN = os.environ.get("BOT_TOKEN", CONFIG.get("bot_token", "8488696730:AAHwPPhJnan8pJyHTE4xwBhWYsv_p-zjDAo"))

if not API_ID or not API_HASH:
    print("تأكد من تعيين API_ID و API_HASH في المتغيرات أو ملف config.json")
    print("سيتم استخدام القيم الافتراضية للتجربة")
    API_ID = 2040
...
async def show_commands(event):
    try:
        ...
        sections = {
            "م1": ("🛡️ الإدارة والكروبات", [".رفع مشرف", ".رفع مالك", ".اخفاء", ".تنزيل مشرف", ".حظر", ".الغاء حظر", ".كتم", ".الغاء كتم", ".طرد", ".تفليش", ".تصفير", ".الاعضاء", ".المشرفين", ".المعلومات", ".المحذوفين", ".مسح المحظورين", ".غادر", ".تاك", ".الاحداث", ".البوتات"]),
            "م2": ("🎮 الألعاب والترفيه", [".كت", ".نسبه الحب", ".سبام", ".مكرر", ".فصخ", ".ايقاف مكرر"]),
            "م3": ("⚙️ الأساسية والإعدادات", [".الاوامر", ".مساعده", ".ا", ".ايدي", ".ايديي", ".اسمي", ".كشف", ".id", ".stat", ".رابط الحساب", ".المطور", ".جلسة", ".فحص", ".تنظيف"]),
            "م4": ("🔧 المتقدمة والإعدادات", [".اضف_فار", ".حذف_فار", ".فار", ".كل_الفارات", ".اضف متحكم", ".حذف متحكم", ".المتحكمين"]),
            "م5": ("⏰ الوقتية والمزامنة", [".تفعيل الوقت", ".تعطيل الوقت", ".صوره وقتيه", ".اسم وقتي", ".بايو وقتي", ".ايقاف"]),
            "م6": ("💥 الإضافة والتفليش", [".قفل", ".فتح", ".الحاله", ".غبان", ".الغاء غبان"]),
            "م7": ("🤖 البحث والوسائط", [".بحث", ".فيديو", ".اغنية"]),
            "م8": ("📦 التخزين والأرشفة", [".نسخ احتياطي", ".انشاء مجموعة تخزين"]),
            "م9": ("📤 تحويل ورفع الملفات", [".تحميل صوتي", ".تحميل فيد", ".صوتي"]),
            "م10": ("🎭 انتحال الهوية", [".تغيير اسمي", ".تغيير بايو", ".تغيير صورتي", ".حذف صورتي"]),
            "م11": ("📢 الإعلانات", [".اعلان", ".إعلان"]),
            "م15": ("👑 المطور", [".تنصيب", ".تنصيب جلسة", ".اعادة تشغيل", ".تحديث", ".اخر تحديث"]),
            "م16": ("📁 إنشاء ومغادرة المجموعات", [".انشاء مجموعة", ".انشاء قناة", ".المجموعات", ".القنوات"]),
            "م19": ("🧩 أوامر متنوعة", [".الكاشف", ".كاشف", ".الذاتيه", ".تفعيل الذاتيه", ".تعطيل الذاتيه", ".ذاتيه"]),
            "م23": ("📣 التاغ والمنشن", [".تاك"]),
            "م24": ("🔐 حفظ الذاتية", [".الذاتيه", ".تفعيل الذاتيه", ".تعطيل الذاتيه", ".ذاتيه"]),
        }

        header = (
            "ᯓ 𝗦𝗼𝗿𝘀𝗲 𝗔𝗹𝗻𝗶𝘀𝗿 𝗔𝗹𝗮𝘀𝘄𝗮𝗱 - قائمـة الأوامـر 𓆪\n"
            "⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
            "⎆ جميـع الأوامـر أدناه مسجلة داخل السورس.\n"
            "⎆ ضـع النقطة (.) قبل الأمر.\n"
            "⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        )

        chunks, current = [], header
        for key, (title, commands) in sections.items():
        ...
        await event.reply(f"❌ خطأ في قائمة الأوامر: {e}")

# =====================================================================
#                        معالج الأزرار للأوامر
# =====================================================================

@client.on(events.CallbackQuery(pattern=b"cmd_m(\\d+)"))
async def cmd_handler(event):
    num = event.data_match.group(1).decode('utf-8')
    
    commands = {
        "1": """
⎆ أوامـر الإدارة والكروبـات :
...
"""
    ...
    cmd_text = commands.get(num, "❌ الأمر غير موجود")
    
    await event.edit(cmd_text)
    await event.answer("✅ تم النسخ التلقائي للأوامر", alert=True)
